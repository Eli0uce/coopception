...
const Scenario = (() => {
  ...
  const SCENES = {
    intro: {
      location: 'STATION ZÉRO — ORBITE EUROPA — 03:47 UTC', bg: 'space',
      lines: [
        ...
        { speaker:'ARIA',     role:'tech',   text:'Réactiver manuellement les modules critiques. Moi j\'ai les manuels numériques — toi tu as les panneaux de contrôle.' },
    ...
    cutscene_1: {
      location: 'MODULE RÉACTEUR — STABILISÉ', bg: 'green',
      lines: [
    ...
    cutscene_2: {
      location: 'MODULE ATMOSPHÈRE — NORMALISÉ', bg: 'green',
      lines: [
        ...
        { speaker:'ARIA',  role:'tech', text:'Je ne sais pas. Mais les communications partielles sont rétablies. Si on les restaure, on pourrait retrouver qui a fait ça.' },
      ]
    },
    cutscene_3: {
      location: 'MODULE COMMUNICATIONS — RÉTABLI', bg: 'amber',
      lines: [
    ...
    cutscene_4: {
      location: 'MODULE PROPULSION — OPÉRATIONNEL', bg: 'green',
      lines: [
    ...
    outro_win: {
      location: 'STATION ZÉRO — TOUS MODULES OPÉRATIONNELS', bg: 'win',
      lines: [
        ...
        { speaker:'ARIA',    role:'tech',   text:'Je n\'arrive pas à y croire. Pile à temps.' },
    ...
    outro_lose: {
      location: 'STATION ZÉRO — PROTOCOLE ÉCHOUÉ', bg: 'lose',
      lines: [
  ...
  // ── Sync Firebase ─────────────────────────────────────────────────────────
  let _writeLineFn  = null;   // (sceneId, lineIdx) => void
  let _listenLineFn = null;   // (sceneId, cb) => stopFn
  let _stopListenFn = null;
  let _isController = true;

  function initSync(writeLineFn, listenLineFn) {
    _writeLineFn  = writeLineFn;
    _listenLineFn = listenLineFn;
  }
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .cin-header {
        ...
        margin-bottom:16px;font-size:11px;letter-spacing:2px;color:var(--green-dim);
      }
      .cin-location { color:var(--amber); }
      .cin-box {
        width:min(760px,95vw);
        border:1px solid var(--green);background:rgba(0,10,0,0.9);
        box-shadow:0 0 30px rgba(0,255,65,0.15);position:relative;
      }
      .cin-box.amber-theme { border-color:var(--amber); box-shadow:0 0 30px rgba(255,176,0,0.15); }
      .cin-body { display:flex;align-items:flex-start;gap:20px;padding:24px;min-height:130px; }
      .cin-portrait { display:flex;flex-direction:column;align-items:center;gap:8px;flex-shrink:0; }
      .cin-avatar {
        width:64px;height:64px;border:2px solid;
        display:flex;align-items:center;justify-content:center;
        font-size:26px;position:relative;overflow:hidden;
      }
      .cin-avatar::before { content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,0.05),transparent); }
      .cin-avatar.tech  { border-color:var(--green); background:rgba(0,255,65,0.08); }
      .cin-avatar.tech::after  { content:'🖥'; font-size:30px; }
      .cin-avatar.oper  { border-color:var(--amber); background:rgba(255,176,0,0.08); }
      .cin-avatar.oper::after  { content:'⚙'; font-size:30px; }
      .cin-avatar.system{ border-color:var(--red); background:rgba(255,51,51,0.08); animation:cin-alert .5s step-end infinite; }
      .cin-avatar.system::after{ content:'⚠'; font-size:30px; }
      @keyframes cin-alert { 0%,100%{opacity:1} 50%{opacity:0.4} }
      .cin-name { font-size:11px;letter-spacing:3px;padding:2px 8px;border:1px solid; }
      ...
      .cin-dialogue { flex:1; }
      .cin-text { font-size:16px;color:#e8e8e8;line-height:1.7;min-height:80px;letter-spacing:0.5px; }
      .cin-cursor { display:inline-block;color:var(--green);animation:blink .7s step-end infinite;margin-left:2px; }
      .cin-footer {
        display:flex;align-items:center;justify-content:space-between;
        padding:10px 24px;border-top:1px solid var(--border);background:rgba(0,0,0,0.5);
      }
      .cin-progress { font-size:11px;color:var(--green-dim);letter-spacing:2px; }
      .cin-controls { display:flex;gap:10px;align-items:center; }
      .cin-btn { background:transparent;font-family:'Share Tech Mono',monospace;font-size:12px;letter-spacing:2px;padding:6px 14px;cursor:pointer;transition:all .15s; }
      .cin-btn-next { border:1px solid var(--green);color:var(--green); }
      .cin-btn-next:hover { background:var(--green);color:#000;box-shadow:var(--glow); }
      .cin-btn-skip { border:1px solid var(--border);color:var(--green-dim); }
      .cin-btn-skip:hover { border-color:var(--red);color:var(--red); }
      .cin-waiting { font-size:11px;color:var(--amber);letter-spacing:2px;animation:blink 1s step-end infinite; }
      .cin-progress-bar { height:2px;background:var(--border); }
      .cin-progress-fill { height:100%;background:var(--green);transition:width .4s ease; }
    `;
      ...
      <div class="cin-box" id="cin-box">
        ...
        <div class="cin-footer">
          ...
          <div class="cin-controls">
            ...
            <span class="cin-waiting" id="cin-waiting" style="display:none">⏳ EN ATTENTE DU TECHNICIEN...</span>
          </div>
        </div>
      </div>
    `;
  ...
  function onKey(e) {
    ...
    if (!_isController) return; // Opérateur ne peut pas avancer
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); nextLine(); }
    if (e.code === 'Escape') skipScene();
  }

  // ── Lecture d'une scène ───────────────────────────────────────────────────
  function play(sceneId, cb, options = {}) {
    const scene = SCENES[sceneId];
    if (!scene) { if (cb) cb(); return; }

    _isController  = options.isController !== false;
    buildOverlay();
    onComplete    = cb;
    ...
    // Appel du callback onStart (pour pause timer)
    if (options.onStart) options.onStart();

    // ── Contrôleur (Technicien) ───────────────────────────────────────────
    if (_isController) {
      document.getElementById('cin-next').style.display = '';
      document.getElementById('cin-skip').style.display = '';
      document.getElementById('cin-waiting').style.display = 'none';
      showLine(0);
      if (_writeLineFn) _writeLineFn(sceneId, 0);

    // ── Spectateur (Opérateur) ────────────────────────────────────────────
    } else {
      document.getElementById('cin-next').style.display = 'none';
      document.getElementById('cin-skip').style.display = 'none';
      document.getElementById('cin-waiting').style.display = 'inline';

      // Nettoyer l'ancien listener
      if (_stopListenFn) { _stopListenFn(); _stopListenFn = null; }

      if (_listenLineFn) {
        _stopListenFn = _listenLineFn(sceneId, lineIdx => {
          if (lineIdx === -1) { // Signal de skip du Technicien
            _stopAndFinish();
  ...
  function showLine(idx) {
    ...
    typeTimer = setInterval(() => {
      if (i < line.text.length) {
        ...
        if (i % 4 === 0 && typeof AudioManager !== 'undefined') AudioManager.play('typeBeep');
      } else {
        clearInterval(typeTimer);
  ...
  function nextLine() {
    ...
    // Écrire dans Firebase AVANT d'afficher (l'Opérateur reçoit le signal)
    if (_writeLineFn) _writeLineFn(currentSceneId, nextIdx);
    showLine(nextIdx);
  }

  // ── Fin de scène ─────────────────────────────────────────────────────────
  function finishScene() {
    // Signaler la fin à l'Opérateur via Firebase
    if (_isController && _writeLineFn) {
      _writeLineFn(currentSceneId, currentLines.length); // index hors-bornes = fin
    }
    if (_stopListenFn) { _stopListenFn(); _stopListenFn = null; }
    setTimeout(() => {
      overlay.style.display = 'none';
  ...
  function skipScene() {
    if (!_isController) return; // L'Opérateur ne peut pas passer
    if (typeof AudioManager !== 'undefined') AudioManager.click();
    if (_writeLineFn) _writeLineFn(currentSceneId, -1); // Signal skip
    clearInterval(typeTimer);
    if (_stopListenFn) { _stopListenFn(); _stopListenFn = null; }
    overlay.style.display = 'none';
    if (onComplete) { const cb = onComplete; onComplete = null; cb(); }
  ...
  // ── API publique ──────────────────────────────────────────────────────────
  return { initSync, play, skip: skipScene };
})();
